...
std::tuple<State, float, float> SpecificWorker::spiral_method(const RoboCompLidar3D::TPoints& filter_data) {
    ...
    const float a = 0.0f;               // mm (radio inicial)
    ...
    std::size_t end   = filter_data.size()/2 + 20;
    auto front_min = std::min_element(filter_data.begin()+start, filter_data.begin()+end,
                                      [](const auto& a, const auto& b){ return a.r < b.r; });

    if(front_min->r < min_distance)
    {
    ...
    static float b = 180.0f;              // mm/rad (separación entre vueltas)
